...
export function App() {
  ...
  React.useEffect(() => {
    try { localStorage.removeItem("bhoomiiq_sidebar_pinned"); } catch { /* ignore */ }
  }, []);

  const handleLogin = (userData) => {
    setCurrentUser(userData);
    try { localStorage.setItem("bhoomiiq_auth_session", JSON.stringify(userData)); } catch (err) { console.error(err); }
  };
  const handleLogout = () => {
    setCurrentUser(null);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans relative">
      <TopNav
        ...
        onReturnToTop={() => { setActiveTab("cadastral"); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
      />

      {!isSidebarOpen && (
        <button type="button" onClick={() => setIsSidebarOpen(true)} className="fixed left-0 top-1/2 -translate-y-1/2 bg-[#080D1A] hover:bg-slate-800 text-slate-400 hover:text-white border border-l-0 border-slate-700/80 px-1.5 py-3.5 rounded-r-xl shadow-xl z-30 transition-all duration-150 group flex flex-col items-center gap-1.5 cursor-pointer" title="Open Modules">
