...
export function GISMap({ projects = [], selectedProjectId, onSelectProject, selectedRisk, baselineRisk }) {
  ...
  useEffect(() => {
    ...
    projects.forEach(project => {
      ...
      marker.bindPopup(`<strong>${id ?? 'Project'}</strong><br/>${project.district ?? ''}, ${project.state ?? ''}${hasRisk ? `<br/>Risk: ${risk.toFixed(1)}/100` : '<br/>Risk: Select project to calculate'}<br/>Stage: ${project.acquisition_stage ?? '—'}${simulationNote}`);
