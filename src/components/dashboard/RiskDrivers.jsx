...
export function RiskDrivers({ drivers = [] }) {
  const maxImpact = Math.max(...drivers.map(d => Math.abs(d.impact)), 0.25);
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.03)] p-5 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-3 pt-3">
        {drivers.map((driver, idx) => {
          ...
          return (
            <div key={idx} className="p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition" title={driver.description}>
