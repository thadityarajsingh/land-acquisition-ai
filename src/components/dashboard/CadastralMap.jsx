    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <span className="font-mono text-xs font-bold text-slate-900">parcel.gutNo ?? parcel.parcel_id ?? '—'</span>
      ...
      {visibleParcels.length === 0 && (
        <div className="flex items-center gap-2 p-5 text-xs text-slate-500">
          <AlertTriangle className="h-4 w-4" /> No official cadastral parcel records are loaded. Add the project-area GeoJSON to render real boundaries.
        </div>
      )}
    </section>
  );
