    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">Parcel details are synchronized with the GIS overlay above.</p>
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <span className="font-mono text-xs font-bold text-slate-900">Gut {parcel.gutNo}</span>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-[10px] text-slate-500">
                ...
                <span>Compensation<br /><b className="text-slate-700">{formatINR(parcel.awardedCompensation)}</b></span>
              </div>
            </button>
          );
        })}
      ...
      {parcels.length > visibleParcels.length && (
        <div className="border-t border-slate-100 px-4 py-2 text-[10px] text-slate-500">
          Showing the first {visibleParcels.length} parcels here. Select parcels directly on the GIS map above for spatial inspection.
        </div>
      )}

      {visibleParcels.length === 0 && (
        <div className="flex items-center gap-2 p-5 text-xs text-slate-500">
          <AlertTriangle className="h-4 w-4" /> No cadastral parcel records are available for this project.
        </div>
      )}
    </section>
  );
