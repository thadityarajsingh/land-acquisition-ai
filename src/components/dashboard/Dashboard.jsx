...
import { CadastralMap } from './CadastralMapGeo';
    ...
    <div className="space-y-5 animate-view-fade-in">
      <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)] card-hover space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="text-xs font-bold font-mono px-2 py-0.5 rounded-md bg-blue-50 text-[#1E3A8A] border border-blue-200/80">{projectData?.id || 'IN-MH-PUN-2024-0094B'}</span>
              <span className="text-xs font-semibold text-slate-500 flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400" />{projectData?.district || 'Pune, Maharashtra'} • {projectData?.corridor || 'Land Acquisition'}</span>
              <span className="text-[11px] bg-emerald-50 text-emerald-700 px-2.5 py-0.5 rounded-full font-semibold border border-emerald-200/80 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>Active Cadastral Sync</span>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-slate-100">
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60"><div className="flex items-center justify-between text-[11px] text-slate-500 font-medium mb-1"><span>Risk Probability</span><span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span></div><div className="flex items-baseline gap-1.5"><span className="text-2xl font-black text-slate-900 font-mono">{currentRiskScore}</span><span className="text-xs font-semibold text-rose-600 font-mono">/ 100</span></div><div className="text-[10px] text-slate-400 mt-0.5">{prediction?.riskCategory || 'Model risk category'}</div></div>
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60"><div className="flex items-center justify-between text-[11px] text-slate-500 font-medium mb-1"><span>Land Area</span><Layers className="w-3.5 h-3.5 text-blue-500" /></div><div className="flex items-baseline gap-1.5"><span className="text-2xl font-black text-slate-900 font-mono">{projectData?.land_area_acres ?? '—'}</span><span className="text-xs font-semibold text-slate-500">Acres</span></div><div className="text-[10px] text-slate-400 mt-0.5">{projectData?.affected_families ?? '—'} affected families</div></div>
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60"><div className="flex items-center justify-between text-[11px] text-slate-500 font-medium mb-1"><span>Approval Delay</span><Clock className="w-3.5 h-3.5 text-rose-500" /></div><div className="flex items-baseline gap-1.5"><span className="text-2xl font-black text-rose-600 font-mono">{projectData?.approval_delay_days ?? '—'}</span><span className="text-xs font-semibold text-slate-500">Days</span></div><div className="text-[10px] text-slate-400 mt-0.5">Recorded project delay input</div></div>
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60"><div className="flex items-center justify-between text-[11px] text-slate-500 font-medium mb-1"><span>Historical Delays</span><IndianRupee className="w-3.5 h-3.5 text-amber-500" /></div><div className="flex items-baseline gap-1.5"><span className="text-2xl font-black text-slate-900 font-mono">{projectData?.historical_delay_count ?? '—'}</span><span className="text-xs font-semibold text-slate-500">Cases</span></div><div className="text-[10px] text-slate-400 mt-0.5">Historical project signal</div></div>
        </div>
      </div>

      {activeTab === 'cadastral' && <div className="space-y-5 animate-view-fade-in">
        <section className="card-hover rounded-2xl"><CadastralMap projectId={projectData?.id} projectName={projectData?.name} district={projectData?.district} projectType={projectData?.corridor} latitude={projectData?.latitude} longitude={projectData?.longitude} parcels={filteredParcels.length > 0 ? filteredParcels : projectData?.parcels || []} selectedGut={selectedParcel?.gutNo} onSelectParcel={setSelectedParcel} /></section>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5"><div className="lg:col-span-5 space-y-5"><div className="card-hover rounded-2xl"><RiskScoreCard score={currentRiskScore} projectData={projectData} simulationDelta={whatIfResult ? whatIfResult.scoreDelta : null} /></div><div className="card-hover rounded-2xl"><RiskDrivers drivers={currentDrivers} /></div></div><div className="lg:col-span-7 space-y-5"><div className="card-hover rounded-2xl"><WhatIfPanel defaults={projectData?.simulationDefaults || {}} onRunSimulation={onRunSimulation} simulating={simulating} onReset={onResetSimulation} /></div><div className="card-hover rounded-2xl"><ComparisonView baseline={projectData} simulation={whatIfResult} /></div></div></div>
