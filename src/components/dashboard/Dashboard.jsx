    ...
    <div className="space-y-5 animate-view-fade-in">
      <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)] card-hover space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div>
            ...
            <h1 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">
              {projectData?.name || 'Land Acquisition Risk Assessment'}
            </h1>
          </div>
          ...
          <div className="text-left lg:text-right">
            ...
            <div className="text-xs font-bold text-slate-800">
              {projectData?.acquisition_stage || 'Assessment'}
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-slate-100">
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60 hover:bg-slate-100/60 transition-all duration-200">
            ...
            <div className="text-[10px] text-slate-400 mt-0.5">{prediction?.riskCategory || 'Model risk category'}</div>
          </div>
          ...
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60 hover:bg-slate-100/60 transition-all duration-200">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 font-mono tracking-tight">{projectData?.land_area_acres ?? '—'}</span>
          ...
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60 hover:bg-slate-100/60 transition-all duration-200">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-rose-600 font-mono tracking-tight">{projectData?.approval_delay_days ?? '—'}</span>
          ...
          <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/60 hover:bg-slate-100/60 transition-all duration-200">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-slate-900 font-mono tracking-tight">{projectData?.historical_delay_count ?? '—'}</span>
        ...
        <div className="space-y-5 animate-view-fade-in">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-7 space-y-5">
              ...
              <div className="card-hover rounded-2xl">
                <ComparisonView baseline={projectData} simulation={whatIfResult} />
              </div>
            </div>
          </div>

          <section className="card-hover rounded-2xl">
            <MitigationProtocols recommendations={recommendations} projectData={projectData} />
          </section>
        </div>
      )}
      ...
      {activeTab === 'corridor' && (
        <div className="animate-view-fade-in">
          <CorridorView projectData={projectData} onSelectParcel={(parcel) => {
      ...
      {activeTab === 'disputes' && (
        <div className="animate-view-fade-in">
          <DisputesView projectData={projectData} />
        </div>
      )}
      ...
      {activeTab === 'audit' && (
        <div className="animate-view-fade-in">
          <AuditTrailView projectData={projectData} />
        </div>
      )}
    </div>
  );
