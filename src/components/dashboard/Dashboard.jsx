    ...
    <div className="space-y-6 animate-view-fade-in">
      <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)] card-hover space-y-5">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="text-[11px] font-bold font-mono px-2.5 py-1 rounded-lg bg-orange-50 text-[#C2410C] border border-orange-200/80 transition-colors">{projectData?.id || 'IN-MH-PUN-2024-0094B'}</span>
              <span className="text-xs font-medium text-slate-500 flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400" />{projectData?.district || 'Pune, Maharashtra'} • {projectData?.corridor || 'Pune Ring Road'}</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-slate-100">
          <div className="bg-slate-50/70 p-4 rounded-2xl border border-slate-200/70 hover:bg-slate-100/60 transition-all duration-200"><div className="flex items-center justify-between text-[11px] text-slate-500 font-medium mb-1"><span>Risk Probability</span><span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span></div><div className="flex items-baseline gap-1.5"><span className="text-2xl font-extrabold text-slate-900 font-mono tracking-tight transition-all duration-300">{currentRiskScore}</span><span className="text-xs font-semibold text-rose-600 font-mono">/ 100</span></div><div className="text-[10px] text-slate-400 mt-0.5">{getRiskLabel(currentRiskScore, prediction?.riskCategory)}</div></div>
