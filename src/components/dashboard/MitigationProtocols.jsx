...
export function MitigationProtocols({ recommendations = [], projectData }) {
  ...
  const handleInitiate = (recId) => {
    setInitiatedActions(prev => ({ ...prev, [recId]: true }));
  };

  const orderedRecommendations = [...recommendations].sort(
    (a, b) => (PRIORITY_ORDER[a.urgency || 'HIGH'] ?? 1) - (PRIORITY_ORDER[b.urgency || 'HIGH'] ?? 1)
  );
  const visibleRecommendations = orderedRecommendations.slice(0, 6);
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 pb-3 border-b border-slate-100">
          <div>
            <div className="flex items-center gap-2">
              ...
              {recommendations.length > 0 && <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-mono">{recommendations.length} active</span>}
            </div>
        ...
        {recommendations.length === 0 ? (
          ...
          <>
              ...
              {visibleRecommendations.map((rec) => {
                ...
                return (
                  <div key={rec.id} className="group relative flex flex-col justify-between p-4 rounded-xl border border-slate-200 hover:border-[#F97316]/60 bg-slate-50/50 hover:bg-white hover:shadow-md transition-all">
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full font-mono ${priority === 'CRITICAL' ? 'bg-rose-100 text-rose-700 border border-rose-200' : priority === 'HIGH' ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-slate-100 text-slate-700 border border-slate-200'}`}>{priority} PRIORITY</span><span className="text-[10px] font-bold font-mono text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200">Risk trigger</span></div>
            ...
            {orderedRecommendations.length > visibleRecommendations.length && <div className="mt-3 text-[10px] text-slate-400 text-center">Showing the 6 highest-priority recommendations; additional rule-triggered actions remain available in the advisory draft.</div>}
          </>
        )}
